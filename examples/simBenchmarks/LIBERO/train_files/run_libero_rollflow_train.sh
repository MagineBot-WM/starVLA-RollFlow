#!/usr/bin/env bash
# Launch the H128/C32 RollFlow LIBERO training job.
#
# The parent process validates the run and creates a detached tmux worker.  The
# worker performs the distributed launch and writes a reproducible copy of this
# script into the run directory.  Override variables with the environment, for
# example: BATCH_PER_GPU=4 RUN_ID=my_run bash "$0".
...
config_yaml="${CONFIG_YAML:-${repo_root}/examples/simBenchmarks/LIBERO/train_files/starvla_qwengroot_rollflow_libero_h128_c32_80k.yaml}"
data_root="${DATA_ROOT:-/data/tzq/datasets/starVLA/Datasets/libero}"
data_mix="${DATA_MIX:-libero_all_rollflow_h128}"
run_root="${RUN_ROOT:-/data/tzq/starVLA_checkpoints}"
run_id="${RUN_ID:-libero_qwengroot_rollflow_h128_c32_80k_unfrozen}"

# ------------------------------ Distributed/training knobs -----------------
num_gpus="${NUM_GPUS:-4}"
cuda_visible_devices="${CUDA_VISIBLE_DEVICES:-0,1,2,3}"
main_process_port="${MAIN_PROCESS_PORT:-29501}"
# H128 is substantially heavier than the H32 baseline; keep the safe default.
...
tmux_session="${TMUX_SESSION:-rollflow_libero_h128}"
...
# Fail early before creating a tmux session or touching a run directory.
for pair in \
  "NUM_GPUS:${num_gpus}" \
...
# Parent mode: start one detached worker and return immediately.
if [[ "${1:-}" != "--worker" ]]; then
  ...
  printf -v worker_cmd \
    'cd %q && exec env STARVLA_DIR=%q STARVLA_PYTHON=%q CONFIG_YAML=%q DATA_ROOT=%q DATA_MIX=%q RUN_ROOT=%q RUN_ID=%q NUM_GPUS=%q CUDA_VISIBLE_DEVICES=%q MAIN_PROCESS_PORT=%q BATCH_PER_GPU=%q GRADIENT_ACCUMULATION_STEPS=%q MAX_TRAIN_STEPS=%q SAVE_INTERVAL=%q EVAL_INTERVAL=%q LOGGING_FREQUENCY=%q WAIT_FOR_GPU_FREE=%q RESUME=%q PYTORCH_CUDA_ALLOC_CONF=%q PYTHONUNBUFFERED=1 NO_ALBUMENTATIONS_UPDATE=1 WANDB_MODE=disabled %q --worker >> %q 2>&1' \
    ...
    "${gradient_accumulation_steps}" "${max_train_steps}" "${save_interval}" \
    "${eval_interval}" "${logging_frequency}" "${wait_for_gpu_free}" "${resume}" \
  ...
  echo "Effective raw batch: $((num_gpus * batch_per_gpu * gradient_accumulation_steps))"
...
# Worker mode: optionally wait for all GPUs, then launch Accelerate/DeepSpeed.
