...
class AdaLayerNorm(nn.Module):
    ...
    def forward(
        ...
        if temb.ndim == 2:
            ...
            raise ValueError(f"temb must have shape [B,D] or [B,T,D], got {tuple(temb.shape)}")
...
class DiT(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # MeanFlow conditions on the terminal time t and interval length t-r.
        # Keeping separate encoders follows the reference implementation while
        # retaining ``timestep_encoder`` for single-time checkpoint compatibility.
        ...
        self.interval_timestep_encoder = TimestepEncoder(
            embedding_dim=self.inner_dim, compute_dtype=compute_dtype
        )
    ...
    def forward(
        ...
        start_timestep: Optional[torch.Tensor] = None,
